...
export default function RegistrarBlockchainAudit() {
    ...
    <div className="min-h-screen bg-gray-50 pl-56 md:pl-64">
      ...
      <div className="max-w-5xl mx-auto p-6">
        ...
        {validity && (
          ...
          >
            {validity.isValid ? '✅ All blocks valid' : '❌ Tampering detected!'} — Total Blocks: {validity.totalBlocks}
          </div>
        )}
          ...
          <div className="space-y-3">
            {chain.map((block) => (
              <div key={block._id} className="bg-white rounded-xl shadow p-4 font-mono text-xs">
                ...
                <p className="text-gray-500">🕐 {new Date(block.timestamp).toLocaleString()}</p>
                <p className="text-green-700 break-all mt-1">Hash: {block.hash}</p>
                <p className="text-gray-400 break-all">Prev: {block.previousHash}</p>
              </div>
            ))}
          </div>
        )}
